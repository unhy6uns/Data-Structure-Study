...
O-표기법 : 트리가 포화상태인 경우  
Omega-표기법 : 가장 깊은 곳에 리프가 딱 하나 있는 경우  
이렇게 두고 병합정렬에서와 같이 증명 가능
# Prob.4
## 1) 선택 정렬
Theta(n^2)
## 2) 버블 정렬
...
Theta(n^2)  
proof.  
$E[T(n)] = \displaystyle \sum_{k=1}^{n-1} \frac{1}{n} k + E[T(n-1)]$  
$= \Theta(n) + E[T(n-1)]$  
...
퀵 정렬 - 정렬된 경우가 최악이다.  
힙 정렬 - 정렬된 경우 buildheap에서 역순으로 힙을 만들어야 한다. 여기서 nlogn의 시간이 걸린다.  
...
생각 좀 더 해보자  
비교연산은 빼기가 필요한데 이 연산 시간이 단순 대입(좌시프트)보다 훨씬 오래 걸림.  
함수 호출하는데 걸리는 시간? (이진 탐색)
...
따라서 안정성을 유지한다.
## 삽입 정렬
newItem이 A[loc]보다 작아질 때까지 삽입이 일어난다.  
뒤쪽에 동일한 원소가 있는 경우 기존의 원소의 뒤쪽에 삽입되므로 순서가 유지된다.  
따라서 안정성을 유지한다.
## 병합 정렬
...
2번 구역을 옮길 때 우쉬프트를 사용하고, 기준원소도 좌시프트로 보내면 안정성을 유지시킬 수 있다.
## 힙 정렬
...
cf.  
stack overflow가 생기는 이유는 재귀함수가 계속 호출되지만 함수가 종료되지는 않아서 변수들이 계속 메모리를 차지하기 때문이다.  
완전 정렬된 경우 logn이 아니라 n번의 재귀함수가 호출된다.
